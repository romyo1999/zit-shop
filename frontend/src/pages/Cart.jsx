...
const Cart = () => {
  ...
  const [pr_info,setPr_info]=useState([]);
  ...
  useEffect(() => {
    ...
    if(products.length<1){
      const timeoutId = setTimeout(() => {
        setMessage("You don't have any  Items in Cart yet ");
  ...
  useEffect(()=>{
    ...
    products.map((e)=>{
        ...
        q+=(e.quantity)
        pr_info.push({"product_id":e.product.id,"size":e.size ,"quantity":e.quantity,"price":e.product.price*e.quantity})
    })
    setItems(q)
    setSubTotal(s)
    setPr_info(pr_info)
...

{
        ...
        products.length>0?(
          ...
          <div className='d-flex align-items-center justify-content-center' style={{fontSize:"20px" ,fontWeight:"bold" ,marginTop:"100px"}}>    
          <div>yuo have no  items here</div>
      </div>
        )
