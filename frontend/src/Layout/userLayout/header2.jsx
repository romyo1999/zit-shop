...
const {reload ,setReload}=useFavoritContext();
            ...
            <div className='header-main '  >
               <Link to="/"  className='header-main text-dark ms-2 ' style={{width:"160px" }}> <img     className="img-fluid" src={zit} alt="shopping-cart--v2"/></Link>
                ...
                <Dropdown.Toggle id="dropdown-basic" style={{ display:"flex",flexDirection:"column" ,alignContent:"center",alignItems:"center",width:"100px" ,height:'60px', background:"white",border:0}}>
                 ...
                 src={user.image ==="images/user.png"?userPhoto:`${backendUrl}${user.image}`} alt="profile" />
                </Dropdown.Toggle>

                <Dropdown.Menu style={{padding:'3px' ,boxShadow:"0 0 5px"}}>
                    <Dropdown.Item className="mb-2 mt-2 p-1 " style={{fontWeight:'bold'}} ><Link className="user-options" to={'/profile'}><FontAwesomeIcon icon={faCog }/> Account Setting</Link></Dropdown.Item>

                    {
                        user.role==="admin"?(
                            <Dropdown.Item  className="mb-2 mt-2 p-1 " style={{fontWeight:'bold'}} ><Link className="user-options" to={"/admin/orders"}><FontAwesomeIcon icon={faChartLine  }/> Admin Dashboard</Link></Dropdown.Item>
                            ):(
                             <span></span>
                        )
                    }

                    <Dropdown.Item  className="mb-2 mt-2 p-1 " style={{fontWeight:'bold'}}  href="/myorders"><Link className="user-options" to={"/myorders"}><FontAwesomeIcon icon={faBoxes}/> My Orders</Link></Dropdown.Item>
                    <Dropdown.Item  className="mb-2 mt-2 p-1 " style={{fontWeight:'bold'}}   onClick={logout} ><FontAwesomeIcon icon={faSignOut}/> SignOut</Dropdown.Item>
                        ...
                        <Link to="login" style={{minWidth:"fit-content",fontWeight:"bold"}} className='ms-2 me-2 text-dark text-decoration-none'><h6 className='mb-0 sign-in'>Sing In</h6></Link>
                    )
                }
